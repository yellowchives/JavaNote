...
假如同时向表中插入10000条数据，会非常慢，因为有下面两个原因：

1. 每次执行一条 sql，获取和释放连接浪费时间
2. mysql默认自动开启事务，会对每条SQL都开启事务然后提交

解决的办法就是批处理：

1. 把要执行的10000条sql打包了，一次性提交给mysql服务器。节省了频繁创建和释放连接的时间
2. 手动开启事务，等10000条sql执行完成后再手动提交

```java
@Test
    public void batchInsert() throws ClassNotFoundException, SQLException {
        Class.forName("com.mysql.cj.jdbc.Driver");
        Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/batch", "root", "root");
        conn.setAutoCommit(false); //关闭自动提交
        String sql = "insert into user (name) values (?)";
        PreparedStatement pstm = conn.prepareStatement(sql); //获取预定义处理对象
        //添加10000条sql语句
