...
address:
  - beijing
  - shanghai
# 注意@Value一次只能注入数组的一个值，想要一次性注入数组的值需要@ConfigurationProperties(prefix = "address")
# 行内写法
...
**Evironment**:springboot自动生成的，包含了配置文件中所有的属性
...
## **10-SpringBoot配置-获取数据_2**
 ...
 @ConfigurationProperties 一次性把变量值注入到类的field中

**注意**：prefix一定要写。这样会根据变量名自动往一个类里注入值

如果配置不在application.yml中，还需要使用@PropertyResource("classpath:a.yml")来指定配置文件的位置
...
public class Person {
    ...
    private String name; //这个名字要和application.yml中的一样
    private int age;
...
- 命令行参数：java –jar xxx.jar  --spring.profiles.active=dev。要先把springboot项目打包成install了。

## **12-SpringBoot配置-项目内部配置文件加载顺序**

 加载顺序为上文的排列顺序，高优先级配置的属性会生效

- file:./config/：当前项目下的/config目录下。**注意是项目的目录，不是模块的目录。**
- file:./           ：当前项目的根目录
- classpath:/config/：classpath的/config目录
- classpath:/  ：classpath的根目录。**resources目录里的文件会把打包到这里。**

## **13-SpringBoot配置-项目外部配置加载顺序**

 外部配置文件的使用是为了不修改内部配置文件就能修改运行的配置环境
